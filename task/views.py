...
def task_pending(request):
    tareas = Task.objects.filter(user = request.user, datecomplete__isnull=True).order_by('-datecomplete')
    return render(request,'task_pending.html',{
        'task' : tareas
    })
...
def task_detail(request,task_id):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request,'task_detail.html',{'task' :task, 'form': form, 'error':'Error al actulizar el registro'})
